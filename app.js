(() => {
  ...
  BABYLON.SceneLoader.ImportMeshAsync('','./assets/','James_NEXUS_Animated.glb',scene).then(result=>{
    ...
    log('Office 1.43 · scharf · Pathfinding V2 + direct Door Driver aktiv');
  ...
  scene.onBeforeRenderObservable.add(()=>{
    ...
    // Door mechanics now run in this exact render loop, together with James.
    if(window.NEXUS_AUTO_DOORS && typeof window.NEXUS_AUTO_DOORS.tick==='function'){
      const p=(typeof jamesRoot.getAbsolutePosition==='function')?jamesRoot.getAbsolutePosition():jamesRoot.position;
      window.NEXUS_AUTO_DOORS.tick(p,!!travel,frameDt);
    }
